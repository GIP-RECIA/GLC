...
import ChipsFilter from '@/components/old-ui/filter/ChipsFilter.vue'
...
const tags = {
  category: [
    {
      id: CategoriePersonne.Enseignant,
    ...
    {
      id: CategoriePersonne.Non_enseignant_service_academique,
      i18n: 'person.category.nonTeacherAcademicService',
    },
    {
      id: CategoriePersonne.Eleve,
  ...
  type: [
    {
      id: '',
      i18n: 'source.official',
    },
    {
      id: 'SarapisUi_',
      i18n: 'source.SarapisUi_',
    },
  ],
}
...
function filter(): void {
  ...
  const searchValue = filters.value.search
  ...
  if (!isEmpty(filters.value.categories)) {
    result = result
      .filter(personne => filters.value.categories.includes(personne.categorie))
  }

  if (!isEmpty(filters.value.status)) {
    result = result
      .filter(personne => filters.value.status.includes(personne.etat))
  }

  if (!isEmpty(filters.value.types)) {
    result = result.filter(personne =>
      filters.value.types.includes(
