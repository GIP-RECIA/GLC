...
public class AuthenticatedUserEvaluation implements IEvaluation, Serializable {
  ...
  public boolean isApplicable(UserDTO userInfos) {
    ...
    final boolean authenticated = (auth != null) && auth.isAuthenticated() && auth.getPrincipal().equals(userInfos.getUserId());
    if (log.isDebugEnabled())
      log.debug("AuthenticatedUserEvaluation on {} with auth state {}", userInfos.getUserId(), auth.toString());
