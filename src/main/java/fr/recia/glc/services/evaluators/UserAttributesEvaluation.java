...
public class UserAttributesEvaluation implements IEvaluation, Serializable {
  ...
  public boolean isApplicable(@NotNull final UserDTO userInfos) {
    ...
    final List<String> attribs = userInfos.getAttributes().get(this.attribute);
    if (attribs == null || attribs.isEmpty()) return false;

    if (attribs.size() > 1)
      throw new IllegalArgumentException("Multivalued Attribute Evaluator should be used on '" + this.attribute + "' ");
    ...
    if (StringEvaluationMode.EQUALS.equals(mode)) return attrib.equals(value);
    if (StringEvaluationMode.EXISTS.equals(mode)) return attrib != null;
    if (StringEvaluationMode.STARTS_WITH.equals(mode)) return attrib.startsWith(value);
    if (StringEvaluationMode.ENDS_WITH.equals(mode)) return attrib.endsWith(value);
    if (StringEvaluationMode.CONTAINS.equals(mode)) return (attrib.indexOf(value) != -1);
    if (StringEvaluationMode.MATCH.equals(mode)) return attrib.matches(value);
    // will never get here
    return false;
  }
